...
def plot_trajectory(filepath: str, outfile_path="data.csv", save2csv=True) -> None:
    ...
    for i in range(len(phases)):
        ...
        stu_action.append(phases[i]['stu_action'].cpu().numpy().squeeze())

        if phases[i]['tea_action'] is None:
            tea_action.append(([0., 0., 0., 0., 0., 0.]))
        else:
            tea_action.append(phases[i]['tea_action'].tolist())
    ...
    tea_action = np.array(tea_action)
    stu_action = np.array(stu_action)
    stu_action = np.expand_dims(stu_action, axis=-1)
    # print(f"stu_action: {stu_action}")
    # print(f"tea_action: {tea_action}")
    tea_action = np.where(tea_action is None, 0., tea_action)
    # tea_action = tea_action.squeeze()
    # tea_action = np.expand_dims(tea_action, axis=-1)
    action_mode = np.array(action_mode)
    motor_power = np.array(motor_power)

    action_mode = action_mode.squeeze()
    stu_action = stu_action.squeeze()
    tea_action = tea_action.squeeze()
    ...
    axes2.plot(step, stu_action[:, 0], zorder=3, label='stu_vx')
    axes2.plot(step, tea_action[:, 0], zorder=2, label='tea_vx')
    ...
    axes2.plot(step, stu_action[:, 1], zorder=3, label='stu_vy')
    axes2.plot(step, tea_action[:, 1], zorder=2, label='tea_vy')
    ...
    axes2.plot(step, stu_action[:, 2], zorder=3, label='stu_vz')
    axes2.plot(step, tea_action[:, 2], zorder=2, label='tea_vz')
    ...
    axes2.plot(step, stu_action[:, 3], zorder=3, label='stu_wx')
    axes2.plot(step, tea_action[:, 3], zorder=2, label='tea_wx')
    ...
    axes2.plot(step, stu_action[:, 4], zorder=3, label='stu_wy')
    axes2.plot(step, tea_action[:, 4], zorder=2, label='tea_wy')
    ...
    axes2.plot(step, stu_action[:, 5], zorder=3, label='stu_wz')
    axes2.plot(step, tea_action[:, 5], zorder=2, label='tea_wz')
    ...
    axes3.plot(step, stu_action[:, 0], zorder=3, label='stu_vx')
    axes3.plot(step, tea_action[:, 0], zorder=2, label='tea_vx')
    ...
    if save2csv:
        ...
        data = {
            ...
            'lyapunov_energy': lyapunov_energy.squeeze(),
            'motor_power': motor_power.squeeze(),
            'motor_energy_consumption': stepwise_energy.squeeze(),
        }
...
if __name__ == '__main__':
    ...
    # Self-defined file
    # fp = f"logs/eval/uneven_rtl_suboptimal.pkl"
    # fp = f"logs/eval/src-learning.pkl"
    # fp = f"logs/eval/uneven_only_teacher_larger_chi.pkl"
