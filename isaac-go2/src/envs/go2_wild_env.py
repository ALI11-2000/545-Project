...
class Go2WildExploreEnv:
    ...
    def __init__(self,
        ...
        # Trigger
        self.trigger = Trigger(num_envs=self._num_envs, trigger_cfg=config.trigger, device=self._device)

        # PHY-Teacher
        self.phy_teacher = PHYTeacher(num_envs=self._num_envs, teacher_cfg=config.phy_teacher, device=self._device)
        ...
        self.curr_goal_distance = torch.zeros(self._num_envs, device=self._device)  # Current Goal Distance
        self.last_goal_distance = torch.zeros(self._num_envs, device=self._device)  # Last Goal Distance
    ...
    def reset_idx(self, env_ids) -> torch.Tensor:
        ...
        if env_ids.shape[0] > 0:
            ...
            self.curr_goal_distance[env_ids] = 0
            self.last_goal_distance[env_ids] = 0
    ...
    def step(self, drl_action: torch.Tensor):
        ...
        for step in range(max(int(self._config.env_dt / self._robot.control_timestep), 1)):
            ...
            # drl_action_res = drl_action
            # phy_action_res = self._solved_acc * 0.
            stu_action = drl_action_res + phy_action_res
            # print(f"drl_action_res: {drl_action_res}")
            # print(f"phy_action_res: {phy_action_res}")
            # print(f"stu_action: {stu_action}")
            # print(f"drl action: {drl_action}")
            # import pdb
            # pdb.set_trace()

            # PHY-Teacher update
            self._robot.energy_2d = self.phy_teacher.update(self._torque_optimizer.tracking_error)

            # PHY-Teacher action
            tea_action, dwell_flag = self.phy_teacher.get_action()

            self._terminal_stance_ddq, action_mode = self.trigger.get_terminal_action(
            ...
            logging.info(f"stu_action: {stu_action}")
            logging.info(f"tea_action: {tea_action}")
            ...
            # Action mode indices
            student_indices = torch.argwhere(action_mode == ActionMode.STUDENT.value).squeeze(-1)
            teacher_indices = torch.argwhere(action_mode == ActionMode.TEACHER.value).squeeze(-1)
            stu_motor_action = None
            tea_motor_action = None
            ...
            if len(student_indices) > 0:
                stu_motor_action, self._desired_acc[student_indices], self._solved_acc[student_indices], \
                    ...
                    foot_contact_state=self._gait_generator.desired_contact_state[student_indices],
                    swing_foot_position=self._swing_leg_controller.desired_foot_positions[student_indices],
                    generated_acc=self._terminal_stance_ddq[student_indices]
                )
            ...
            if len(teacher_indices) > 0:
                tea_motor_action, self._desired_acc[teacher_indices], self._solved_acc[teacher_indices], \
                    ...
                    foot_contact_state=self._gait_generator.desired_contact_state[teacher_indices],
                    swing_foot_position=self._swing_leg_controller.desired_foot_positions[teacher_indices],
                    generated_acc=self._terminal_stance_ddq[teacher_indices],
                    gravity_frame=True
                )
                self._teacher_activation_times[teacher_indices] += 1
                # self.robot.set_robot_base_color(color=(1., 0, 0), env_ids=teacher_indices)  # Display Red
                # Nominal actions for DRL-student
                # nominal_actions[teacher_indices] = (tea_action[teacher_indices] - phy_action_res[teacher_indices]) / weights
                nominal_actions[teacher_indices] = tea_action[teacher_indices] - phy_action_res[teacher_indices]
            ...
            logs.append(
                dict(timestamp=self._robot.time_since_reset,
                     ...
                     acc_min=self.acceleration_space[0],
                     acc_max=self.acceleration_space[1],
            ...
            # Sum reward
            if self._planner.planning_flag is False:  # Finish navigation
                sum_reward += rewards
            else:
        ...
        self.last_goal_distance = self.curr_goal_distance
        self.curr_goal_distance = self.get_goal_distance

        end = time.time()
        logging.info(f"***************** step duration: {end - start} *****************")

        return self._obs_buf, self._privileged_obs_buf, nominal_actions, action_mode, sum_reward, dones, self._extras

    def _get_observations(self):
        distance_obs = self.get_goal_distance.unsqueeze(dim=-1)
    ...
    def _is_fail(self):
        ...
        is_fail = torch.logical_or(
            ...
            self._robot.base_position[:, 2] < self._config.get('check_fall_on_height', 0.12))

        # Robot roll threshold
        is_fail = torch.logical_or(is_fail, self._robot.base_orientation_rpy[:, 0] >
                                   self._config.get('check_fall_on_roll', 1.745))

        # Robot pitch threshold
        is_fail = torch.logical_or(is_fail, self._robot.base_orientation_rpy[:, 1] >
                                   self._config.get('check_fall_on_pitch', np.pi / 2))
        return is_fail
    ...
    @property
    def acceleration_space(self):
        return (to_torch([-10, -10, -10, -20, -20, -20], device=self._device),
