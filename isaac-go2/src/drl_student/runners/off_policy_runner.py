...
class OffPolicyRunner:
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        ep_infos = []
        failed_times = 0
        avg_ep_return = 0  # Episodic return
        avg_ep_length = 0  # Episodic length
        avg_step_reward = 0  # Stepwise reward
        teacher_cnt = 0
        student_cnt = 0
        activation_ratio = 0.0
        mean_value_loss, mean_surrogate_loss = 100, 100

        rewbuffer = deque(maxlen=100)
        lenbuffer = deque(maxlen=100)
        cur_reward_sum = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device)
        cur_episode_length = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device)
        activation_steps = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device)
        ...
        for it in tqdm(range(self.current_learning_iteration, total_iter)):
            ...
            for i in range(self.num_steps_per_env):
                ...
                if self.log_dir is not None:
                    ...
                    activation_steps += action_mode  # Activation steps
                    ...
                    if new_ids.numel() > 0:
                        avg_ep_return = cur_reward_sum[new_ids][:, 0].mean().clone().item()  # Episodic return
                        avg_ep_length = cur_episode_length[new_ids][:, 0].mean().item()  # Episodic length
                        avg_step_reward = avg_ep_return / avg_ep_length  # Average Stepwise reward
                        teacher_cnt = activation_steps[new_ids][:, 0].mean().clone().item()
                        student_cnt = (cur_episode_length[new_ids] - activation_steps[new_ids])[:,
                                      0].mean().clone().item()
                        activation_ratio = teacher_cnt / (teacher_cnt + student_cnt)
                ...
                # Save dataset to replay storage
                dataset = self.alg.to_transition(prev_obs=prev_obs, obs=obs, actions=actions, rewards=rewards,
                                                 dones=dones, infos=infos)

                # Decide whether to perform DDPG update
                need_update = False
                if self.env.phy_teacher.teacher_correct.any():
                    need_update = True
                elif action_mode.sum() == 0:
                    need_update = True

                if need_update:
                    stats = self.alg.update([dataset], action_mode)
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar('Perf/teacher_count', locs['teacher_cnt'], locs['it'])
        self.writer.add_scalar('Perf/student_count', locs['student_cnt'], locs['it'])
        self.writer.add_scalar('Perf/activation_ratio', locs['activation_ratio'], locs['it'])
        self.writer.add_scalar('Train/avg_episode_return', locs['avg_ep_return'], locs['it'])
        self.writer.add_scalar('Train/avg_episode_return/time', locs['avg_ep_return'], self.total_time)
        self.writer.add_scalar('Train/avg_step_reward', locs['avg_step_reward'], locs['it'])
        self.writer.add_scalar('Train/avg_step_reward/time', locs['avg_step_reward'], self.total_time)
