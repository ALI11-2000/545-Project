...
from src.drl_student.runners import OffPolicyRunner

from src.configs.training import ddpg, sac
...
flags.DEFINE_bool("enable_phy_teacher", True, "whether to enable the PHY-Teacher.")
...
def main(argv):
    ...
    # Use DDPG
    config.training = sac.get_training_config()
    ...
    if FLAGS.enable_phy_teacher:
        ...
        # config.environment.phy_teacher.tau = 100
    ...
    sac_runner = OffPolicyRunner(env, config.training, logdir, device=device)
    if FLAGS.load_checkpoint:
        sac_runner.load(FLAGS.load_checkpoint)
    sac_runner.learn(num_learning_iterations=config.training.runner.max_iterations,
